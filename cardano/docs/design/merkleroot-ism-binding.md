...
`verified_message_nft(mailbox_policy_id)` bakes in the mailbox state-NFT policy, and the mailbox bakes in the resulting `verified_message_nft_policy` — a mutual parameterization. **Whenever the mailbox is redeployed, the relayer/agent config's `verifiedMessageNftScriptCbor` + `verifiedMessageNftPolicyId` must be regenerated** from the new deployment; a stale value makes every inbound delivery fail with a bare Plutus `error` (the policy can't find "its" mailbox in the tx inputs). Take both from `deployments/preview/verified_message_nft_applied.plutus` (the applied CBOR); the policy id is `blake2b_224(0x03 ‖ <applied cbor bytes>)` for PlutusV3. This bit the first inbound e2e run. The same applies to **recipients** (e.g. `greeting`), which also bake `verified_message_nft_policy` — a stale recipient can't recognize the delivered NFT and its `HandleMessage` errors; redeploy the recipient (`init recipient`, new address) after a mailbox change.

## 13. How to exercise the MerkleRoot ISM in a test env (and why steady-state uses MessageId)

**Steady state on preview uses a MessageId default ISM** with a recent `index.from`: no tree, no backfill, and the ~870k historical Sepolia→2003 messages are never indexed, so there is no delivery flood. MerkleRoot is fully proven (§10) but is *not* the day-to-day default because exercising it against the **shared** official Sepolia mailbox is expensive.

To exercise MerkleRoot **inbound** (Sepolia→Cardano) you need one of:

1. **A fresh, dedicated Sepolia mailbox + your own Sepolia validator.** Zero history → no flood, and the tree is tiny → proofs build in seconds. The official validators only sign the official mailbox, so a fresh mailbox needs an own validator whose address the Cardano ISM trusts. (A fresh core + validator key were prepared in `DeployFreshSepoliaCore.s.sol`-style scripts / `validator-sepolia` config during this work.)
2. **The shared official mailbox + full tree backfill + a `whitelist`.** Set `index.from` to the origin MerkleTreeHook deploy block, use a large-`getLogs` RPC (Tenderly, `chunk` ≈ 99 999), and **wipe `relayer_db`** so the cursor re-runs the backfill. Add a `whitelist` scoped to your test recipient/message so the message cursor doesn't retry the historical backlog.

**Why the flood happens (it is not a protocol defect):** Hyperlane messages are addressed to a `(domain, recipient)`, not to a specific mailbox, and relayers dedup via the destination's *delivered* set. We reused **domain 2003** across many Cardano mailbox redeploys, so a freshly-deployed Cardano mailbox has an empty delivered-set and every historical domain-2003 message looks undeliverable-but-pending; those messages target recipients/ISM configs from replaced deployments, so they fail metadata build and retry forever. A **stable** mailbox (never redeployed) or a **fresh dedicated origin** both avoid this; `index.from` alone cannot, because it governs future indexing, not the already-stored backlog, and MerkleRoot forces `index.from` down to tree genesis anyway.
