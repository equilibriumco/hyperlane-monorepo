...
> The shipped preview deployment has **no generic recipient deployed** — its only
> message consumer is the warp route. `greeting.ak` below is the reference
> implementation, and `hyperlane-cardano init recipient` deploys one if you want
> to exercise the generic inbound path.
...
```bash
BLOCKFROST_API_KEY=your_api_key hyperlane-cardano \
...
Those are the values configured on preview today: 3 gwei assumed Sepolia gas
price, 7500 ADA per ETH, and no overhead — the sender's declared `--gas-limit`
is the whole basis. At 300,000 gas that quotes 6.75 ADA, which is 0.0009 ETH
converted at the configured rate. Re-derive both as prices drift; they are
owner-set numbers, not a live feed.
...
upside is that the rate stays human-readable: 7500 ADA per ETH is written `7500`.
...
**The announced location must match `checkpointSyncer`.** A relayer discovers
checkpoints only through this announcement, so a mismatch means messages are
signed and never delivered, with nothing failing near the cause.

| Syncer | Announce as | Note |
| --- | --- | --- |
| `s3` | `s3://<bucket>/<region>/<folder>` | Region is part of the path, not a separate field |
| `gcs` | `gs://<bucket>/<folder>` | |
| `localStorage` | `file:///<path>` | Only a relayer that can read that path will deliver |

```bash
# Production: publicly readable, so any relayer can pick the checkpoints up
...
# The demo: a docker volume shared with its own relayer, so no cloud account
hyperlane-cardano validator announce \
  --storage-location "file:///data/checkpoints" \
  --validator-key "$CARDANO_VALIDATOR_KEY"
```

A relayer additionally needs `allowLocalCheckpointSyncers: true` before it will
honour a `file://` announcement; it ignores them by default.

Announcing again with the same validator key **updates** the location rather
...
A live deployment on Cardano preview is bridged to Ethereum Sepolia by an
ADA ↔ wADA warp route, and everything needed to drive it ships in `cardano/demo/`.

- **Run it:** [demo/runbook.md](../demo/runbook.md) — start the agents, move
  tokens both ways, reconcile balances.
- **Add a route:** [demo/runbook-warp-routes.md](../demo/runbook-warp-routes.md).
- **Deploy your own core:** [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md).

The Sepolia side is four contracts, deployed by the Forge scripts in
`cardano/demo/solidity/`:

| Contract | Script | Purpose |
| --- | --- | --- |
| `StaticMessageIdMultisigIsm` | `DeployCardanoISM.s.sol` | Verifies Cardano validator signatures on inbound messages |
| `InterchainGasPaymaster` + oracle | `DeploySepoliaIGP.s.sol` | Prices Sepolia → Cardano delivery |
| `StaticAggregationHook` | `DeployAggregationHook.s.sol` | Merkle tree hook + IGP, so dispatches both prove and pay |
| `HypERC20` (wADA) | `DeployWAdaRoute.s.sol` | The synthetic token, ISM and hook wired, Cardano router enrolled |

`EnrollCardanoRouters.s.sol` enrols Cardano routers on existing Sepolia routes
when you need to do that separately.

Run them from `cardano/demo/solidity`, which is its own Foundry project — it
remaps back into the monorepo's `solidity/` for contracts and `forge-std`, so
`solidity/dependencies/` must exist first:
...
| Validator Agent                         | Tested         | Signing checkpoints; local or S3/GCS storage |
| Warp Routes (Native, Collateral, Synth) | Tested         | All three flavours; the shipped demo runs native ↔ synthetic |
...
| Scraper                                 | Implemented    | Indexes dispatches, deliveries and gas payments into Postgres |
| Explorer                                | Implemented    | Renders Cardano addresses, transactions and warp routes |
...
- Validator Agent: checkpoint signing, local / S3 / GCS storage
- Scraper: Cardano indexer plus the migration that inserts the Cardano domains,
  so messages, deliveries and gas payments land in the same schema every other
  Hyperlane chain uses
- Explorer: Cardano address and transaction rendering, warp route resolution and
  a server-side route balance read (in the `cardano` branch of our fork)

### What's Missing

1. **On-chain custom ISM enforcement**: the mailbox reads the recipient's ISM
   from its datum, but a recipient cannot yet force its own ISM against a
   relayer that ignores it — enforcement is relayer-side.
2. **Metrics**: the chain crate exposes almost none, so Cardano is close to
   invisible on the standard agent dashboards. Reorg detection reports through
   logs only.
3. **TypeScript SDK**: Cardano is wired into the Rust agents and the Cardano CLI,
   not into `@hyperlane-xyz/sdk`, so `hyperlane warp deploy` and the rest of the
   TS tooling do not target it.
4. **Provider redundancy**: Blockfrost is the sole source of truth, including for
   an agent that signs checkpoints. There is no fallback and no cross-check.

### Recommended Next Steps

**High Priority (Production):**
1. Security audit (Aiken contracts, signature verification, merkle tree)
2. Provider abstraction — see "Provider redundancy" above
