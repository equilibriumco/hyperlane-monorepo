...
> [CARDANO_GUIDE.md §7](CARDANO_GUIDE.md#7-interchain-gas-paymaster-igp).
...
- [demo/runbook.md](../demo/runbook.md) — start the stack, transfer both
  directions, reconcile balances, troubleshoot.
- [demo/runbook-warp-routes.md](../demo/runbook-warp-routes.md) — add a warp route
  to a live deployment, both sides, with an end-of-page checklist.
...
aiken --version  # built and tested against v1.1.21

# 2. Rust toolchain (for the CLI)
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
rustup default stable

# 3. Install the Hyperlane Cardano CLI
cargo install --path cardano/cli --locked
hyperlane-cardano --version
```

`--locked` is required: without it `cargo install` re-resolves dependencies and
picks up a `pallas-txbuilder` this code does not compile against. Every
`hyperlane-cardano` command below assumes the installed binary is on `PATH`.
...
hyperlane-cardano --network $NETWORK init mailbox --domain 2003

# Skip waiting (faster for exploratory use)
hyperlane-cardano --no-wait --network $NETWORK init mailbox --domain 2003
```
...
Expected output — one `.spend` or `.mint` per validator, each paired with an
`.else` fallback that Aiken generates:

```
"canonical_config_nft.canonical_config_nft.mint"
...
"synthetic_token.synthetic_token.mint"
"test_token.test_token.mint"
"validator_announce.validator_announce.spend"
"vault.vault.spend"
...
- `--validators`: ISM validators per origin domain. Format: `"domain:addr1,addr2;domain2:addr3"` (20-byte / 40-hex-char EVM addresses; a `0x` prefix is accepted and stripped).
- `--thresholds`: ISM threshold per origin domain. Format: `"domain:threshold;domain2:threshold"`.
- `--storage-location`: where the **Cardano validator's** checkpoints live, announced on-chain so relayers can find them, for the outbound (Cardano → remote) direction. `s3://<bucket>/<region>/<folder>` — the region is part of the path, not a separate field — or `file:///<path>` for a local syncer, as the demo stack uses. It must match the validator config's `checkpointSyncer`; see [CARDANO_GUIDE.md](CARDANO_GUIDE.md#validator-announcement).
...
On success the command prints the applied **State NFT Policy ID** and the state
UTXO it created, and writes both into `deployments/$NETWORK/deployment_info.json`.
Read them back from there rather than transcribing them:

```bash
jq '.mailbox | {hash, address, stateNftPolicy, utxo}' deployments/$NETWORK/deployment_info.json
```
...
hyperlane-cardano \
  ...
  --network $NETWORK \
  init igp \
  --oracle "11155111:3000000000:7500:0"
```
...
- `--oracle`: repeatable gas oracle config per remote domain, format `"domain:gas_price:exchange_rate:gas_overhead"`. The values above are what preview runs today: 3 gwei assumed Sepolia gas price, 7500 ADA per ETH, and no overhead, so the sender's declared gas limit is the whole basis. See the **Gas Payment (IGP) Configuration & Enforcement** appendix for how they are derived, the matching Sepolia-side oracle setup, and enabling relayer enforcement.
...
- `--remote-decimals`: the decimals the **paired remote token** uses. This is
  not a free choice — it must equal the remote token's own `decimals`, because
  the two describe the same wire amount from opposite ends. EVM tokens are
  commonly 18, but the shipped demo pairs ADA with a 6-decimal wADA so that no
  rescaling happens at all. Mismatch it and every transfer arrives silently
  scaled by a power of ten.

**Output:** the command prints `Warp Route Deployment Complete!` with the route's
script hash, **NFT policy**, address and reference-script UTXO, and appends the
route to `deployment_info.json`. Read it back rather than transcribing:

```bash
jq '.warp_routes[-1]' deployments/$NETWORK/deployment_info.json
```

The NFT policy is the route's identity. Its Hyperlane address —
`0x01000000` ‖ nft_policy — is what you enroll on remote chains:

```bash
echo "0x01000000$(jq -r '.warp_routes[-1].nftPolicy' deployments/$NETWORK/deployment_info.json)"
```
...
Route appended to deployments/preview/deployment_info.json (.warp_routes)
```
...
```bash
# Read the route's NFT policy back out of the deployment
WARP_POLICY=$(jq -r '.warp_routes[] | select(.warpType=="native") | .nftPolicy' \
  deployments/$NETWORK/deployment_info.json)
...
```bash
docker compose -f demo/docker/docker-compose.yml logs -f relayer
```
...
```bash
# Read the synthetic route's minting policy and decimals
SYNTH=$(jq -r '.warp_routes[] | select(.warpType=="synthetic")' \
  deployments/$NETWORK/deployment_info.json)
SYNTH_POLICY=$(echo "$SYNTH" | jq -r '.mintingPolicy')
SYNTH_DECIMALS=$(echo "$SYNTH" | jq -r '.decimals')
...
```bash
SYNTH_WARP=$(jq -r '.warp_routes[] | select(.warpType=="synthetic")' deployments/$NETWORK/deployment_info.json)
WARP_POLICY=$(echo "$SYNTH_WARP" | jq -r '.nftPolicy')
```
...
docker compose -f demo/docker/docker-compose.yml exec relayer cat /config/relayer.json
...
NATIVE_WARP=$(jq -r '.warp_routes[] | select(.warpType=="native")' deployments/$NETWORK/deployment_info.json)
NATIVE_SCRIPT_HASH=$(echo "$NATIVE_WARP" | jq -r '.scriptHash')
NATIVE_NFT_POLICY=$(echo "$NATIVE_WARP" | jq -r '.nftPolicy')
...
the agents' `.env` is stale until you refresh it.** The `demo/docker` relayer and
validator configs are `${VAR}` templates resolved from `cardano/demo/docker/.env`
...
```bash
grep -E 'CARDANO_(MAILBOX|ISM|IGP)_SCRIPT_HASH|CARDANO_INDEX_FROM' ../../demo/docker/.env
...
```bash
cd cardano/demo/docker
docker compose down -v      # -v also drops the named volumes
```

`-v` removes the relayer's message DB, the validator's checkpoint DB, the signed
checkpoints and the scraper's Postgres in one step. Naming the volumes
individually is not worth it — they are prefixed with the compose project name,
which is derived from the directory and changes if the stack is moved.
...
```bash
# Query mailbox state
hyperlane-cardano --network $NETWORK query mailbox

# Query ISM configuration
hyperlane-cardano --network $NETWORK query ism

# Query UTXOs at an address
hyperlane-cardano --network $NETWORK query utxos <address>

# Query specific UTXO
hyperlane-cardano --network $NETWORK query utxo <tx_hash>#<index>

# Get protocol parameters
hyperlane-cardano --network $NETWORK query params
```
...
```bash
hyperlane-cardano --signing-key $CARDANO_SIGNING_KEY --network $NETWORK \
...
export VALIDATOR_KEY="$CARDANO_VALIDATOR_KEY"   # from demo/docker/.env, or your own
...
BLOCKFROST_API_KEY=$BLOCKFROST_API_KEY \
$CLI --signing-key $CARDANO_SIGNING_KEY --network $NETWORK \
  init igp --oracle "11155111:3000000000:7500:0"
...
# what demo/docker/.env expects, and verifiedMessageNftScriptCbor/warp entries
...
echo "Next: refresh demo/docker/.env from this deployment with the block in"
echo "  'Phase 8.1: Refresh .env from the deployment', then start the agents."
```

---

## Appendix: the shipped preview deployment

`deployments/preview/deployment_info.json` records the live deployment this
repository ships against. Read it rather than copying addresses out of a
document — it is what the CLI itself uses, and it is updated by the deploy
commands.

```bash
jq '{mailbox: .mailbox.address, ism: .ism.address, igp: .igp.address,
     va: .validator_announce.address,
     routes: [.warp_routes[] | {(.warpType): .address}]}' \
  deployments/preview/deployment_info.json
```

Each contract entry carries its `hash` (the script hash), `address` (bech32),
`stateNft` (the policy authenticating its state UTXO) and `referenceScriptUtxo`
(where the on-chain copy of the script lives, so transactions can reference it
instead of embedding it).

Before relying on any of it, confirm it is still intact — reference-script UTXOs
are spendable and this is shared testnet state:

```bash
demo/scripts/preflight.sh
```

The Sepolia counterparts live in `demo/docker/.env.example`, pre-filled.
...
```bash
hyperlane-cardano init all --domain 2003
```
...
```bash
hyperlane-cardano init recipient \
  --custom-contracts ./contracts \
  --custom-module greeting \
  --custom-validator greeting
```
...
hyperlane-cardano --network $NETWORK \
...
> **`config generate-env` caveat**: treat its output as a *starting point*, not a drop-in `.env`. Its variable names drift from what the `demo/docker` stack's `.env` actually expects — e.g. it emits `CARDANO_VERIFIED_MSG_POLICY_ID` with no accompanying CBOR field, it can carry stale/duplicate warp route entries from earlier deployments, and it leaves placeholders for `CARDANO_SIGNER_KEY` / `CARDANO_INDEX_FROM`. Reconcile the generated file by hand against the **Environment Variables Overview** below and `demo/docker/.env.example` before using it. In particular, the H256 address forms the agents actually read are:
>
...
Checkpoint storage is set by `checkpointSyncer` in the validator's config, not by
environment variables. The demo stack uses `localStorage` on a docker volume
shared with the relayer, so it needs no cloud account at all; a production
validator uses `s3` (with `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`,
`AWS_REGION`, `AWS_S3_BUCKET`) or `gcs`, so that any relayer can read the
checkpoints. Whichever you choose must match the location announced on-chain by
...
> **Tip**: Use `config generate-env` as a starting point, but its variable names drift from what `demo/docker/.env` expects — see the caveat above. The manual extraction below produces the values the agents actually need.
...
**Recipients bake in the same policy — redeploy them too.** `verified_message_nft` isn't only read by the relayer; every recipient contract (including `greeting`) is itself parameterized by `verified_message_nft_policy` (see [Script Parameterization Table](#script-parameterization-table)). After a mailbox redeploy, a recipient deployed against the *old* policy can never recognize the NFT the relayer delivers — there is no config value to patch, the policy is baked into the recipient's own script hash and address. Redeploy the recipient (`init recipient` again — it gets a new script address) whenever the mailbox is redeployed.
...
```bash
# The demo's key lives in cardano/demo/docker/.env; source it, or use your own
set -a; . cardano/demo/docker/.env; set +a
...
forge script DeploySepoliaIGP.s.sol:DeploySepoliaIGP \
  --rpc-url $EVM_RPC_URL \
  --broadcast \
  --private-key $EVM_SIGNER_KEY 2>&1 | tee /tmp/step.out
...
forge script DeployAggregationHook.s.sol:DeployAggregationHook \
  --rpc-url $EVM_RPC_URL \
  --broadcast \
  --private-key $EVM_SIGNER_KEY 2>&1 | tee /tmp/step.out
...
### Step 2: Deploy the Sepolia warp route

[`DeployWAdaRoute.s.sol`](../demo/solidity/DeployWAdaRoute.s.sol) deploys the
synthetic token, points it at the Cardano ISM, wires the aggregation hook and
enrolls the Cardano router — all in one run. Deploy the Cardano side first, since
enrolling needs its address.

#### Required environment variables

| Variable | From |
| --- | --- |
| `SEPOLIA_SIGNER_KEY` | your funded Sepolia key |
| `SEPOLIA_MAILBOX` | the official Sepolia mailbox |
| `SEPOLIA_ISM` | Step 1 (`DeployCardanoISM.s.sol`) |
| `SEPOLIA_AGGREGATION_HOOK` | Step 1b (`DeployAggregationHook.s.sol`) |
| `CARDANO_NATIVE_WARP_ROUTE` | `0x01000000` + the Cardano route's NFT policy |
| `CARDANO_DOMAIN` | optional, defaults to 2003 |
...
It prints `SEPOLIA_SYNTHETIC_WADA=0x…`. Put that in `demo/docker/.env`.

To bridge a different token, copy the script and change the name, symbol,
decimals and scale. The decimals must match the Cardano route's
`--remote-decimals`, or every transfer arrives scaled by a power of ten — see
[runbook-warp-routes.md](../demo/runbook-warp-routes.md).

### Step 3: Enroll the Cardano side

Step 2 taught Sepolia about Cardano; the reverse is a Cardano transaction:
...
bytes. Verify both directions:

```bash
cast call "$SEPOLIA_SYNTHETIC_WADA" 'routers(uint32)(bytes32)' 2003 \
  --rpc-url "$SEPOLIA_RPC_URL"
hyperlane-cardano warp show --warp-policy <cardano-route-nft-policy>
```

### Step 4: Verify the wiring

```bash
# The route must use the Cardano ISM, not the mailbox default
cast call "$SEPOLIA_SYNTHETIC_WADA" 'interchainSecurityModule()(address)' --rpc-url "$SEPOLIA_RPC_URL"

# ...and the aggregation hook, or dispatches never pay the IGP
cast call "$SEPOLIA_SYNTHETIC_WADA" 'hook()(address)' --rpc-url "$SEPOLIA_RPC_URL"

# The ISM must trust your Cardano validator
cast call "$SEPOLIA_ISM" 'validatorsAndThreshold(bytes)(address[],uint8)' 0x --rpc-url "$SEPOLIA_RPC_URL"
cast wallet address --private-key "$CARDANO_VALIDATOR_KEY"   # must appear above
```

### Step 5: Test a transfer

Both directions, plus how to read the result, are in
...
transfer. Full rationale: **[CARDANO_GUIDE.md §7](CARDANO_GUIDE.md#7-interchain-gas-paymaster-igp)**.
...
```bash
# What preview runs today: 3 gwei assumed Sepolia gas price, 7500 ADA per ETH,
# no overhead — so the sender's declared --gas-limit is the whole basis.
hyperlane-cardano igp set-oracle \
  --domain 11155111 \
  --gas-price 3000000000 \
  --exchange-rate 7500

# verify: 300000 gas should quote 6.75 ADA
hyperlane-cardano igp quote --destination 11155111 --gas-limit 300000
hyperlane-cardano igp show
```

Sanity-check the arithmetic rather than trusting the numbers: 300,000 gas at
3 gwei is 0.0009 ETH, which at 7500 ADA/ETH is 6.75 ADA. Both figures are
owner-set and static, not a live feed, so re-derive them as prices drift.

Adding a `--gas-overhead` shifts margin from the sender's declared limit into a
fixed per-message charge; the demo leaves it at 0 and sizes the limit instead.
...
> the rate stays human-readable: 7500 ADA per ETH is written `7500`.
...
#    (1.395e18 converts lovelace -> wei; re-derive if the ADA/ETH rate moves)
...
- **Cardano → Sepolia** (`warp transfer`): pass `--gas-limit` so the transfer
  bundles the IGP payment. With the Cardano overhead at 0, `--gas-limit` is the
  entire basis and must cover the delivery. `mailbox dispatch` has **no** gas option
...
Use the provided Forge script at `cardano/demo/solidity/DeployAggregationHook.s.sol`:
