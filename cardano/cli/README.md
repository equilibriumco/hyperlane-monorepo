...
picks up a `pallas-txbuilder` this code does not compile against.

To build without installing, `cargo build --release` inside `cardano/cli` leaves
the binary at `target/release/hyperlane-cardano`.
...
# The demo ships all of these pre-filled; sourcing it is enough to run the
# commands below from cardano/ with no flags at all:
...
# Show the route's configuration, and its enrolled remote routers
hyperlane-cardano warp show --warp-policy <nft-policy>
hyperlane-cardano warp routers --warp-policy <nft-policy>
```
