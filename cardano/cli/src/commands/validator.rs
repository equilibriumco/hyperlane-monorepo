...
use crate::utils::crypto::script_hash_from_hex;
...
/// Load the applied `validator_announce` recorded by a previous deployment.
///
/// Re-applying parameters reproduces the deployed script only while
/// `contracts/plutus.json` still holds the blueprint it was built from. Once the
/// contracts move on, re-applying yields a *different* script hash — and so a
/// different script address, at which the live announcements do not exist. The
/// recorded deployment is authoritative, so prefer the applied script on disk
/// whenever it still hashes to the hash the deployment recorded.
fn load_recorded_validator_announce(
    ctx: &CliContext,
    recorded_hash: &str,
) -> Option<AppliedValidator> {
    let path = ctx
        .network_deployments_dir()
        .join("validator_announce_applied.plutus");
    let raw = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&raw).ok()?;
    let cbor = json.get("cborHex")?.as_str()?.to_string();
    let hash = hex::encode(script_hash_from_hex(&cbor).ok()?);
    (hash == recorded_hash).then_some(AppliedValidator {
        compiled_code: cbor,
        policy_id: hash,
    })
}
...
pub(crate) async fn parametrize_validator_announce(
    ...
    let recorded = deployment
        .validator_announce
        .as_ref()
        .and_then(|va| load_recorded_validator_announce(ctx, &va.hash));

    let applied = match recorded {
        Some(applied) => {
            println!("\n{}", "Using recorded validator_announce...".cyan());
            applied
        }
        None => {
            println!("\n{}", "Applying validator parameters...".cyan());
            apply_validator_params(
                &ctx.contracts_dir,
                "validator_announce",
                "validator_announce",
                &[
...
async fn show_announcements(ctx: &CliContext, validator_filter: Option<String>) -> Result<()> {
    ...
    // Prefer the deployed script over re-applying parameters: the deployment is
    // authoritative, and re-applying against newer contracts points `show` at an
    // address where the live announcements do not exist.
    let recorded = deployment
        .validator_announce
        .as_ref()
        .and_then(|va| load_recorded_validator_announce(ctx, &va.hash));
    ...
    let applied = match recorded {
        ...
        None => {
            let policy_id_cbor = encode_policy_id_param(mailbox_policy_id)?;
            let domain_cbor = encode_int_param(local_domain);
            let script_hash_cbor = encode_policy_id_param(&mailbox.hash)?;
            apply_validator_params(
                &ctx.contracts_dir,
                "validator_announce",
                "validator_announce",
