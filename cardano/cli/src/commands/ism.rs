...
enum IsmCommands {
    ...
    /// Migrate ISM state to a new script address
    Migrate {
        /// New ISM script hash (28 bytes, hex). Auto-computed from blueprint if omitted.
        #[arg(long)]
...
pub async fn execute(ctx: &CliContext, args: IsmArgs) -> Result<()> {
    match args.command {
        ...
        IsmCommands::Migrate {
            ...
            signing_key,
            dry_run,
        } => migrate(ctx, new_script_hash, ism_policy, signing_key, dry_run).await,
    }
}
...
async fn migrate(
    ...
    println!("{}", "Migrating ISM to new script address...".cyan());
    ...
    let current_hash_hex = match pallas_addresses::Address::from_bech32(&ism_utxo.address) {
    ...
    if current_hash_hex == new_script_hash {
        println!(
            "\n{}",
            "ISM is already at the target script hash. No migration needed.".yellow()
        );
        return Ok(());
    }
    ...
    if let Ok(mut deployment) = ctx.load_deployment_info() {
        if let Some(ref mut ism) = deployment.ism {
            ism.hash = new_script_hash.clone();
            ism.address = new_address.clone();
