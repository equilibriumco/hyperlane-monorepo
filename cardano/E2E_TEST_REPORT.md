# Cardano ↔ Sepolia E2E Warp Route Test Report

**Date**: 2026-02-12
**Network**: Cardano Preview ↔ Ethereum Sepolia
**Domains**: Cardano Preview (2003) ↔ Sepolia (11155111)

> **Note**: This report was originally produced against Avalanche Fuji and re-labelled for the Sepolia migration. The Sepolia Mailbox address is the current deployed value, but the other EVM contract addresses, transaction hashes, message IDs, and IGP gas/exchange-rate figures below are the original Fuji run artifacts and must be regenerated by re-running the E2E suite on Sepolia.
...
| 2 | Native Warp Transfer | C→S | ADA → WADA | PASS |
| 3 | Native Warp Transfer | S→C | WADA → ADA | PASS |
| 4 | Collateral Warp Transfer | C→S | TEST → WCTEST | PASS |
| 5 | Collateral Warp Transfer | S→C | WCTEST → TEST | PASS |
| 6 | Synthetic Warp Transfer | S→C | FTEST → Synthetic | PASS |
| 7 | Synthetic Warp Transfer | C→S | Synthetic → FTEST | PASS |
...
|-----------|-----------|---------------|---------|
| Cardano→Sepolia | 1,000,000,000 (1 gwei) | 34 | `gas * 1e9 * 34 / 1e12` → lovelace |
| Sepolia→Cardano | 1 | 2.983e20 | `gas * 1 * 2.983e20 / 1e10` → wei |
...
| Relayer (Sepolia) | `0x1f26bfC6f52CbFad5c3fA8dABb71007b28bf4749` |
...
**Cardano IGP** (destination: Sepolia/11155111):
- Gas limit: 150,000, Gas overhead: 0, Total gas: 150,000
- Quote: `150000 * 1e9 * 34 / 1e12 = 5,100 lovelace`

**Sepolia IGP** (destination: Cardano/2003):
- Gas limit: 3,000,000
- Quote: `3000000 * 1 * 2.983e20 / 1e10 = 89,490,000,000,000,000 wei ≈ 0.08949 ETH`

**Result**: PASS

---

## Test 2: Cardano → Sepolia Native (ADA → WADA)
...
| Sepolia Delivery TX | `0xfbddeed3...` |
| Status | Finalized on Sepolia |

**Result**: PASS — 3 WADA minted on Sepolia

---

## Test 3: Sepolia → Cardano Native (WADA → ADA)

| Field | Value |
|-------|-------|
| Amount | 50,000 lovelace (wire: 50,000,000,000,000,000) |
| Sender | Sepolia signer |
| Receiver | Recipient wallet (`6b055f14...`) |
| Message ID | `0xa27d6d76...` |
| Sepolia Dispatch TX | `0x7ccef71d...` |
| Sepolia IGP Payment TX | `0xe461cb18...` (3M gas, ~0.08949 ETH) |
...
## Test 4: Cardano → Sepolia Collateral (TEST → WCTEST)

| Field | Value |
|-------|-------|
| Amount | 50,000 TEST tokens |
| Sender | Recipient wallet |
| Receiver | Sepolia signer |
...
| Sepolia Delivery TX | `0x13d72e0b...` |
| Status | Finalized on Sepolia |

**Result**: PASS — 50,000 WCTEST minted on Sepolia

---

## Test 5: Sepolia → Cardano Collateral (WCTEST → TEST)
...
| Sepolia Dispatch TX | `0xf907e203...` |
| Sepolia IGP Payment TX | `0xa65a3c27...` (3M gas) |
...
## Test 6: Sepolia → Cardano Synthetic (FTEST → Synthetic)
...
| Sepolia Dispatch TX | `0x892ded28ee89a6dd1aab32e36875faeaa73e5e508a0588711c5c6f86001ddc2f` |
| Sepolia IGP Payment TX | `0xf6de0b4e95d4de4f6a241dc0da842fb1734c5840cbeb4bb48e08e65d2147e7c5` |
...
## Test 7: Cardano → Sepolia Synthetic (Synthetic → FTEST)
...
| Sepolia Delivery TX | `0x7616e3cff239a148890c59de892fe65e1dc7871473f8bf4d864c2c25dec0d9a4` |
| Status | Finalized on Sepolia |

**Result**: PASS — 250,000 synthetic tokens burned on Cardano, 0.25 FTEST released on Sepolia
...
**Impact**: Any Sepolia→Cardano synthetic transfer with an amount smaller than `10^(remote_decimals - local_decimals)` raw units would crash the relayer.
...
3. **Atomic IGP payment**: C→S transfers include IGP payment atomically in the same TX
4. **Decimal conversion**: 18-decimal EVM tokens correctly scaled to 6-decimal Cardano tokens and back
5. **Synthetic token minting/burning**: Cardano minting policy correctly mints on S→C and burns on C→S
