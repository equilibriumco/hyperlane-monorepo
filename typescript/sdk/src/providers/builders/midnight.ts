 ...
 */
export const defaultMidnightProviderBuilder: ProviderBuilderFn<MidnightProvider> = (
  metadata: ChainMetadata,
) => {
  const { rpcUrls } = metadata;
  assert(rpcUrls.length > 0, 'Midnight requires at least one rpcUrl (the indexer GraphQL endpoint)');
  return {
