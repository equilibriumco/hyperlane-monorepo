...
use tx_helpers::{
    convert_wire_to_local_amount, credential_to_address, is_retryable_bad_inputs_error,
    parse_address, parse_fee_too_small_expected, parse_policy_id, parse_token_message,
    utxo_to_input,
};
...
use pallas_txbuilder::{
    BuildConway, BuiltTransaction, ExUnits, Output, ScriptKind, StagingTransaction,
};
...
/// Asset name of the ISM state NFT, hex-encoded ("ISM State").
const ISM_STATE_ASSET_NAME_HEX: &str = "49534d205374617465";
...
/// Passes allowed when converging on a fee: writing the fee into the TX changes
/// its size, which changes the required fee.
...
impl HyperlaneTxBuilder {
    ...
    pub async fn build_process_tx(
        ...
        let ism_asset_name = ISM_STATE_ASSET_NAME_HEX;
        ...
        // top-level keys in the value object).
        {
                ...
                .await
            {
                Ok((real_fee, ex_units_map)) => {
                    ...
                    // Writing the fee into the TX changes its serialised size, which
                    // changes the fee the node requires. A single correction pass can
                    // still land short, and the submission then bounces with
                    // FeeTooSmallUTxO. Iterate until the fee covers the TX that carries
                    // it; it settles in a pass or two, and the cap is a backstop against
                    // an encoding boundary that oscillates rather than converging.
                    let total_mem: u64 = ex_units_map.values().map(|(m, _)| m).sum();
                    let total_steps: u64 = ex_units_map.values().map(|(_, s)| s).sum();
                    let fee_params = self.fee_params().await?;

                    actual_fee = real_fee;
                    for pass in 1..=MAX_FEE_CONVERGENCE_PASSES {
                        let needed = fee_for(
                            &fee_params,
                        ...
                        if needed <= actual_fee {
                            break;
                        }
                        info!(
                            "Fee correction (pass {}): {} → {} (TX size changed after rebuild)",
                            pass, actual_fee, needed
                        );
                        let corrected = self
                            .build_complete_process_tx(
                                &components,
                                payer,
                                Some((needed, &ex_units_map)),
                                tracked_payer.as_ref(),
                            )
                            .await?;
                        ...
                        if pass == MAX_FEE_CONVERGENCE_PASSES {
                            warn!(
                                "Fee did not converge in {} passes; submitting at {} and \
                                 relying on the node's expected-fee retry if it is short",
                                MAX_FEE_CONVERGENCE_PASSES, actual_fee
                            );
                        }
                    }
    ...
    fn extract_chained_state(
        ...
        // Locate the outputs by what they carry, not by a predicted position.
        // How many outputs a process TX emits varies with the recipient kind,
        // whether tokens are released and whether a verified-message NFT is
        // minted; an arithmetic layout guess that drifts from what the builder
        // actually emits silently disables TX chaining.
        let utxos = outputs
            .iter()
            .enumerate()
            .map(|(i, o)| self.output_to_utxo(actual_hash, i as u32, o))
            .collect::<Result<Vec<_>, TxBuilderError>>()?;

        // Mailbox continuation is always output #0.
        let mailbox_utxo = utxos
            .first()
            .ok_or_else(|| {
        ...
        // ISM continuation is whichever output carries the ISM state NFT.
        let ism_utxo = utxos
            .iter()
            .find(|u| u.has_asset(&components.ism_state_nft_policy, ISM_STATE_ASSET_NAME_HEX))
            .ok_or_else(|| {
                TxBuilderError::Encoding(format!(
                    "No output carries the ISM state NFT {} (have {} outputs)",
                    components.ism_state_nft_policy,
                    utxos.len()
                ))
            })?
            .clone();
        ...
        let payer_utxo = utxos
            .last()
            .ok_or_else(|| TxBuilderError::Encoding("TX has no change output".to_string()))?
            .clone();

        info!(
            "extract_chained_state: {} outputs in TX, ism at #{}, change at #{} (actual_hash={})",
            utxos.len(),
            ism_utxo.output_index,
            payer_utxo.output_index,
            actual_hash
        );
    ...
    async fn compute_fee_from_evaluation(
        ...
        let params = self.fee_params().await?;
        Ok(fee_for(
            &params,
            tx_size,
            total_mem,
    ...
    /// Fetch the protocol parameters that determine a transaction's fee.
    ///
    /// Kept separate from [`fee_for`] so that converging on a fee costs one
    /// request rather than one per pass — the endpoint is not cached, and this
    /// runs on every delivery.
    async fn fee_params(&self) -> Result<FeeParams, TxBuilderError> {
...
/// Protocol parameters that determine a transaction's fee.
#[derive(Debug, Clone, Copy)]
struct FeeParams {
    min_fee_a: u64,
    min_fee_b: u64,
    price_mem: f64,
    price_step: f64,
    min_fee_ref_script: u64,
}

/// The Conway fee formula: size_fee + script_fee + ref_script_fee, plus a margin
/// for CBOR encoding variance between build passes.
fn fee_for(
    p: &FeeParams,
    tx_size: u64,
    total_mem: u64,
    ...
    let size_fee = p.min_fee_b + (tx_size * p.min_fee_a);
    let script_fee =
        (p.price_mem * total_mem as f64 + p.price_step * total_steps as f64).ceil() as u64;
    let ref_script_fee = p.min_fee_ref_script * ref_script_size;
    ...
    info!(
        ...
        size_fee, script_fee, ref_script_fee, p.min_fee_ref_script, ref_script_size, base_fee, fee_with_margin
    );
...
#[cfg(test)]
mod tests {
    /// Preview's parameters, as of 2026-08.
    fn preview_fee_params() -> FeeParams {
        FeeParams {
            min_fee_a: 44,
            min_fee_b: 155_381,
            price_mem: 0.0577,
            price_step: 0.000_072_1,
            min_fee_ref_script: 15,
        }
    }

    #[test]
    fn fee_matches_the_conway_formula() {
        let p = preview_fee_params();
        // Numbers from a real delivery: 7751-byte TX, 17474 bytes of reference
        // scripts (mailbox 7440 + ISM 3334 + warp route 6700).
        let fee = fee_for(&p, 7751, 9_000_000, 5_000_000_000, 17_474);

        let size_fee = 155_381 + 7751 * 44;
        let script_fee =
            (0.0577_f64 * 9_000_000.0 + 0.000_072_1_f64 * 5_000_000_000.0).ceil() as u64;
        let ref_fee = 15 * 17_474;
        let expected = ((size_fee + script_fee + ref_fee) as f64 * FEE_MARGIN_FACTOR).ceil() as u64;

        assert_eq!(fee, expected);
        assert!(
            fee > size_fee + script_fee + ref_fee,
            "margin must be applied"
        );
    }

    #[test]
    fn fee_grows_with_tx_size_so_convergence_terminates() {
        // The convergence loop relies on the fee being monotonic in TX size and
        // on a fixed size yielding a fixed fee; without both it could oscillate.
        let p = preview_fee_params();
        let small = fee_for(&p, 7000, 1_000_000, 1_000_000_000, 17_474);
        let large = fee_for(&p, 8000, 1_000_000, 1_000_000_000, 17_474);
        assert!(large > small);
        assert_eq!(small, fee_for(&p, 7000, 1_000_000, 1_000_000_000, 17_474));
    }

    #[test]
    fn reference_scripts_are_charged_for() {
        // Reference scripts are billed per byte in Conway; omitting them was a
        // plausible cause of underpayment, so keep the term pinned.
        let p = preview_fee_params();
        let without = fee_for(&p, 7751, 1_000_000, 1_000_000_000, 0);
        let with = fee_for(&p, 7751, 1_000_000, 1_000_000_000, 17_474);

        // Both ends are rounded up independently, so the difference can sit one
        // lovelace either side of the margined reference-script cost.
        let ideal = ((15 * 17_474) as f64 * FEE_MARGIN_FACTOR) as u64;
        let delta = with - without;
        assert!(
            delta.abs_diff(ideal) <= 1,
            "reference scripts contributed {delta}, expected about {ideal}"
        );
    }
