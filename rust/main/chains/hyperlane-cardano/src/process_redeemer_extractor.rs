...
/// Whether a mailbox spend redeemer is the `Process` variant (constructor 1).
///
/// The mailbox UTXO is spent by every mailbox action, not just deliveries — a
/// `Dispatch` is constructor 0 — so a non-`Process` redeemer is ordinary
/// traffic rather than a defect, and must not be reported as malformed.
fn is_process_redeemer(datum: &serde_json::Value) -> bool {
    datum.get("constructor").and_then(|v| v.as_u64()) == Some(1)
}

fn parse_message_id_from_datum(datum: &serde_json::Value, tx_hash: &str) -> Option<[u8; 32]> {
    if !is_process_redeemer(datum) {
        return None;
    }
...
pub async fn extract_process_message_ids(
    ...
    for redeemer in matching {
        ...
        // Mailbox spends that are not deliveries (Dispatch, admin actions) are
        // expected here and are not counted as skipped: every dispatch would
        // otherwise be reported as a malformed redeemer.
        if !is_process_redeemer(&datum) {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn dispatch_redeemer_is_not_a_process_redeemer() {
        // A Dispatch spends the mailbox with constructor 0. It must be
        // recognised as "not a delivery" rather than as a malformed Process,
        // or every outbound message logs a spurious malformed-redeemer warning.
        let dispatch = json!({
            "constructor": 0,
            "fields": [{"bytes": "aabbccdd"}]
        });
        assert!(!is_process_redeemer(&dispatch));
        assert!(is_process_redeemer(&valid_process_datum(&"aa".repeat(32))));
    }
