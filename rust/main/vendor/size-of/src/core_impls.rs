...
// 16 arguments.
//
// VENDORED PATCH (hyperlane-midnight ledger-9.1 migration): upstream size-of
// 0.1.5 lists target-specific ABIs (aapcs/cdecl/win64/sysv64/stdcall/fastcall)
// unconditionally. rustc >= 1.95 rejects `extern "aapcs" fn` etc. on targets
// where they are invalid (e.g. aarch64-apple-darwin) with E0570, and size-of
// 0.1.5 is the newest published version so it cannot be bumped. The ledger-9.1
// decode graph pulls sysinfo 0.39, which forces rustc >= 1.95, so this crate
// must compile there. These impls are never instantiated by the only consumer
// in this workspace (starknet-types-core's felt-size accounting), so keep just
// the ABIs valid on every target. Drop this vendored copy once upstream
// size-of gates its ABI list by target.
impl_function_ptrs! {
    "C",
    "Rust",
    "system",
